...
try {
  ...
  // Keep this run synthetic: do not send analytics beacons from the destination site.
  ...
  for (let i = 0; i < count; i++) {
    ...
    let dest;
    try { dest = new URL(decoded); } catch { continue; }
    if (dest.hostname === TARGET_HOST && dest.pathname.replace(/\/+$/, '/') === TARGET_PATH) {
      found = { link, rank: i + 1, href, decoded };
      break;
    }
  }

  if (!found) throw new Error('target_not_found_on_first_page');
  report.rank = found.rank;

  await found.link.scrollIntoViewIfNeeded();
  const nav = page.waitForURL((u) => {
    try {
      const x = new URL(u.toString());
      return x.hostname === TARGET_HOST && x.pathname.replace(/\/+$/, '/') === TARGET_PATH;
    } catch { return false; }
